...
public class WebConfiguration {
		...
		if (!ALLOWABLE_BOOLEANS.matcher(value).matches()) {
			if (LOGGER.isLoggable(Level.WARNING)) {
				LOGGER.log(Level.WARNING,
						...
						new Object[] { value, param.getQualifiedName(),
								"true|false" });
			}
			return false;
		}
	...
	private void initSetList(ServletContext servletContext) {
		...
		Enumeration e = servletContext.getInitParameterNames(); e
				.hasMoreElements();) {
	...
	private void processJndiEntries(String contextName) {
		/*
		 * This section has been disabled due to incompatibilities with Google
		 * App Engine 1.2.6. Context initialContext = null; try { initialContext
		 * = new InitialContext(); } catch (NamingException ne) { if
		 * (LOGGER.isLoggable(Level.WARNING)) { LOGGER.log(Level.WARNING,
		 * ne.toString(), ne); } }
		 * 
		 * if (initialContext != null) { // process environment entries for
		 * (WebEnvironmentEntry entry : WebEnvironmentEntry.values()) { String
		 * entryName = entry.getQualifiedName(); String value = null;
		 * 
		 * try { value = (String) initialContext.lookup(entryName); } catch
		 * (NamingException root) { if (LOGGER.isLoggable(Level.FINE)) {
		 * LOGGER.fine(root.toString()); } }
		 * 
		 * if (value != null) { if (LOGGER.isLoggable(Level.INFO)) { // special
		 * logic for ClientStateSavingPassword if (!entry
		 * .equals(WebEnvironmentEntry.ClientStateSavingPassword)) { if
		 * (LOGGER.isLoggable(loggingLevel)) { LOGGER.log(loggingLevel,
		 * "jsf.config.webconfig.enventryinfo", new Object[] { contextName,
		 * entryName, value }); } } else { if (LOGGER.isLoggable(loggingLevel))
		 * { LOGGER .log( loggingLevel,
		 * "jsf.config.webconfig.enventry.clientencrypt", contextName); } } }
	...
	private boolean canProcessJndiEntries() {
		...
		/*
		 * This section has been disabled due to incompatibilities with Google
		 * App Engine 1.2.6 try { Util.getCurrentLoader(this)
		 * .loadClass("javax.naming.InitialContext"); } catch (Exception e) {
		 */
		if (LOGGER.isLoggable(Level.FINE)) {
			LOGGER.fine("javax.naming is forced unavailable.  JNDI entries related to Mojarra configuration will not be processed.");
		}
		return false;
		/*
		 * } return true;
